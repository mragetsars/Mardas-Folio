...
- Kept a code block left-aligned when one of its lines is only a Persian comment. Each line resolves its own direction, so such a line right-aligned itself in the middle of left-aligned code and took its indentation with it. The exporter already prints `pre` and `code` left-to-right; the editor now matches, while bidirectional text inside a line still lays out as it prints.
...
### Fixed
- Built the `app` target on macOS when updater artifacts are requested. The macOS updater payload is a `.app.tar.gz`, and only that target produces one: asked for `dmg` alone, Tauri warned that no updater-enabled target was built, deleted the `.app` after building the disk image, and left nothing to collect. It is a build input rather than a release artifact, so the published set is still the DMG, the tarball and its signature.
- Stopped an unconfigured Apple credential being read as a request to sign. The release workflow forwards `APPLE_SIGNING_IDENTITY` whether or not it is set, and Tauri treats a *defined* variable as an instruction to codesign — so an unsigned draft ran `codesign --sign ""` and failed with "The specified item could not be found in the keychain". Both macOS jobs failed that way while the equivalent CI jobs, which never define the variable, built cleanly. Blank signing credentials are now dropped from the build environment, so absent means absent.
- Gave the bundler the updater public key it validates. The committed `tauri.conf.json` keeps `plugins.updater.pubkey` empty so ordinary source builds stay offline, and the Rust side reads the real key from a compile-time variable — but `tauri-plugin-updater` validates the *configured* key as well, and an empty one fails the build with "Missing comment in public key". Every native desktop job failed on it the first time a release ran with an updater key configured, on all four targets. The release build configuration now carries the key and the endpoint, and a test holds it there.
- Installed `packaging` in the two release jobs that need it. `build_offline_bundle.py` and `finalize_release_artifacts.py` both reach `release_provenance`, which imports `packaging`, but neither job installs the project — so the import resolved only on runners that happened to ship it. The Linux and macOS offline bundles failed on it, and `publish-draft`, the last job in the pipeline, would have failed the same way after every build had completed. A test now derives each release script's transitive third-party imports and holds the job that runs it to installing them.
- Installed the PDF rasterizer the release gate depends on. The gate runs the visual QA matrix with `--render-png`, which shells out to `pdftoppm`, but the release workflow never installed `poppler-utils` — so every one of the 22 chunks failed at rasterization and no release could be built. The CI visual QA job had always installed it; the release job never did.
- Made a failing visual audit say so. Both audit scripts wrote their failures to a `failures.txt` inside an output directory that a failed release gate never uploads, and printed nothing, so the gate failed with 22 silent chunks and a bare exit code. They now print each failure to stderr, the matrix runner echoes a failing chunk's captured output, and the gate uploads its evidence even when it fails.
- Removed `bundleVCRuntime` from the Windows bundle configuration. It is a Tauri v1 key that survived the v2 migration, and `tauri-build` rejects an unknown field outright, so the Windows installer job could not compile the shell. It had never had any effect: it was a WiX-only option and this bundle is NSIS. Because `tauri-build` only parses the platform file matching the host, no other job read it. A test now holds the Windows bundle keys to the ones the pinned schema accepts.
- Built the desktop frontend before the native jobs compile the shell. `cargo test` compiles the Tauri binary, and `generate_context!` resolves `frontendDist` at compile time, so `apps/desktop/dist` has to exist before the tests run — but `beforeBuildCommand` only builds it for `tauri build`, which happens two steps later. The four native desktop jobs therefore could not compile on a fresh checkout.
- Restored the tracked placeholder that keeps `apps/desktop/src-tauri/resources/sidecar/` present in a checkout, and stopped staging from deleting it. `tauri.conf.json` declares that directory as a bundle resource and the Tauri build script rejects a resource path that does not exist, so on a fresh checkout `cargo test --locked` failed before running a test and took all four native desktop jobs with it. `stage_desktop_runtime.py` replaces the whole directory, so a local build deleted the placeholder and the deletion was committed; staging now carries it into the replacement, and a test holds it there.
- Stopped source distribution normalization failing on Windows. `normalize_sdist.py` renamed the rebuilt archive over the original while the original was still open for reading: POSIX allows that, Windows answers with a sharing violation and the whole normalization fails. The replacement now happens after the source is closed.
- Stopped desktop runtime staging failing on a runtime built for another platform. `_copy_tree` asked the platform to follow each symlink in order to learn whether its target was a directory, but Windows stores a relative target verbatim and refuses to resolve one written with POSIX separators. It now asks about the path the link names, which `validate_local_symlink` has already proven resolves inside the runtime root — and which is the only thing the answer depends on.
- Stopped the sdist permission test asserting one platform's mode. It hardcoded `0o644`, but Windows maps `chmod` onto a single read-only flag and reports `0o666` for every writable file, so the assertion measured the filesystem rather than the archive. It now checks the mode is handed back unchanged, which is the contract the test is named for.
- Stopped the runtime symlink test asserting one platform's spelling. It compared a raw `os.readlink` against `../libshared.so.1`, which no Windows filesystem can return; the manifest and the archive both record the portable spelling, so the staged link is now checked the same way.
