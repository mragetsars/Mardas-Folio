...
Standalone runtime archives contain a flat top-level application directory and an internal SHA-256 inventory. Manifest schema v2 labels every entry as a regular file or symbolic link. Only declared, relative links that resolve inside the runtime are preserved; `lstat`-based verification rejects undeclared links, traversal through a link, absolute or escaping targets, dangling links, cycles, excessive chains, missing or extra entries, digest/target mismatches, missing sidecar executables, protocol/version mismatches, and runtimes without pinned Chromium. Legacy schema-v1 manifests may describe regular files only. The runtime manifest is integrity evidence, not a digital signature; users should also verify release checksums and GitHub attestations.
...
The standalone document authoring API limits every decoded UTF-8 buffer to 8 MiB and separately bounds imported assets; the project workspace retains its stricter 4 MiB per-file boundary. `document.read`/`document.save` accept Markdown, while `document.read_text`/`document.save_text` accept the supported `.bib`, `.json`, `.toml`, `.txt`, `.yaml`, and `.yml` text types. Read/save responses expose normalized `kind`, `revision`, and `read_only` metadata; saves use same-directory temporary files plus atomic replacement and compare revision tokens before overwriting an existing document. Project read/save responses preserve the same metadata in addition to their SHA-256 conflict token. Asset imports reject symbolic-link selections and symbolic-link `assets/` directories. Recovery is bounded per document, local to the user's application profile, and is never treated as permission to write the source file automatically. Unsaved preview HTML is produced by the existing sanitized Markdown pipeline and is sanitized again before insertion into the desktop DOM.
